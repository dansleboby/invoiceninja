...
class PermissionsCheck
{
    ...
    public function handle(Request $request, Closure $next, $guard = 'user')
    {
        ...
        if(isset($actions['controller'])) {
            $class           = explode('\\', substr($actions['controller'], 0, strpos($actions['controller'], '@')));
            $permission_name = Utils::from_camel_case(str_replace('Controller', '', array_pop($class))) . '.' . explode('@', $actions['controller'])[1];
